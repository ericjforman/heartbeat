...
int Pulse;       // TELLS US WHEN THERE IS A PULSE (TRUE)
...
//THIS FUNCTION IS ALSO IMPORTANT AND NEEDS TO BE CALLED IN SETUP

void initThread()
{
  thread = new MyThread[numThreads];
  
  // create the sound player
    Minim minim = new Minim(this);
    AudioPlayer player = minim.loadFile( "newheartbeat16.aif" );
  
  for(int i = 0; i < numThreads; i++ )
  {
    thread[i] = new MyThread( minim, player );
    thread[i].start();
  }
}

//THIS FUNCTION PLAYS THE BEAT SOUND
void playSound() {   
  
  thread[index].playNow();
